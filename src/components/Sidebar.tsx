...
export const Sidebar: React.FC = () => {
  ...
  const navItems = [
    { name: "Obsidian Website Builder", href: "/", icon: Hexagon, isSpecial: true },
  ...
  return (
    <aside className="fixed top-0 left-0 bottom-0 w-64 bg-white border-r border-slate-200/90 z-40 flex flex-col justify-between p-5 shadow-sm font-sans">
      {/* Brand Header */}
        ...
        <Link href="/" className="flex items-center gap-3 group">
          ...
          <div>
            <span className="font-black tracking-tight text-slate-900 text-lg group-hover:text-indigo-600 transition-colors">
              Obsidian <span className="text-pink-600">AI</span>
            </span>
            <p className="text-[10px] font-mono text-slate-400">Stitch Edition v2.5</p>
          </div>
        </Link>
            ...
            if (isSpecial) {
              return (
                <Link
                  key={item.name}
                  href={item.href}
                  className="flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-xs font-bold text-slate-900 bg-slate-100 hover:bg-slate-200 border border-slate-300/80 transition-all duration-200 shadow-xs mb-3 group"
                >
                  <Hexagon className="w-4 h-4 fill-slate-900 text-slate-900 group-hover:rotate-12 transition-transform" />
                  <span>{item.name}</span>
                </Link>
              );
            }
      ...
      <div className="space-y-4 border-t border-slate-100 pt-4">
        ...
        {user ? (
          <div className="flex items-center justify-between p-2.5 rounded-2xl bg-slate-50 border border-slate-200/80">
            <div className="flex items-center gap-2.5 min-w-0">
              ...
              <div className="min-w-0 flex-1">
                <p className="text-xs font-bold text-slate-900 truncate">{user.name || user.email || "Govind Tripathi"}</p>
                <p className="text-[10px] text-slate-500 truncate">{user.email || "pro-architect@obsidian.dev"}</p>
              </div>
            </div>
            <button
              onClick={() => signOut()}
          ...
          <div className="p-3 rounded-2xl bg-slate-50 border border-slate-200/80 text-center space-y-2">
            <p className="text-xs text-slate-500">Sign in to save your stores.</p>
